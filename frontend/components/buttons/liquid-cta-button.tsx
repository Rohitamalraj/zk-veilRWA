...
export function LiquidCtaButton({ children, className, href, onClick, theme = "dark" }: LiquidCtaButtonProps) {
  ...
  if (href) {
    return (
      <a
        href={href}
        className={cn("group inline-block transition-transform duration-300 hover:scale-105 active:scale-95", className)}
      >
        {ButtonContent}
      </a>
    )
